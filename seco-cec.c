...
int secocec_adap_enable (struct cec_adapter *adap, bool enable);
int secocec_adap_monitor_all_enable (struct cec_adapter *adap, bool enable);
int secocec_adap_log_addr (struct cec_adapter *adap, u8 logical_addr);
int secocec_adap_transmit (struct cec_adapter *adap, u8 attempts,
			   u32 signal_free_time, struct cec_msg *msg);
void secocec_adap_status (struct cec_adapter *adap, struct seq_file *file);

int secocec_received (struct cec_adapter *adap, struct cec_msg *msg);
...
static irq_handler_t secocec_irq_handler(unsigned int irq, void *dev_id, struct
					 pt_regs *regs)
{
	//TODO irq handler
	return 0;
			 ...
			 const struct i2c_device_id *id)
{
	...
	ret = request_irq(secocec->irq,				// The interrupt number requested
			  (irq_handler_t) secocec_irq_handler,  // The pointer to the handler function below
			  IRQflags,				// Use the custom kernel param to set interrupt type
			  dev_name(&client->dev),		// Used in /proc/interrupts to identify the owner
			  dev);					// The *dev_id for shared interrupt lines, NULL is okay
	...
	secocec->cec_adap = cec_allocate_adapter(&secocec_cec_adap_ops,		// opts
						 secocec,			// priv
						 dev_name(dev),	// name
						 opts,				// opts
						 SECOCEC_MAX_ADDRS);		// avail las
